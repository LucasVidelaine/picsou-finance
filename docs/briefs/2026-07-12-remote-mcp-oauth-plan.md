...
- [ ] **Step 2:** Copy the **version-matched** DDL from the Spring Authorization Server release that the BOM resolves (`mvn -q dependency:tree -Dincludes=org.springframework.security:spring-security-oauth2-authorization-server | head` to read the version), from its `oauth2-registered-client-schema.sql`, `oauth2-authorization-schema.sql`, `oauth2-authorization-consent-schema.sql`. Adapt types for PostgreSQL (`blob`→`bytea`, `timestamp`), and note `oauth2_authorization` has very wide `text`/`bytea` columns.
